...
class Model(nn.Module):
    ...
    def __init__(self, input_dim, nclass, pad_index=-1, d_model=512, nhead=8, dim_feedforward=2048, num_encoder_layers=6,
                 dropout=0.1, max_len=512, pool='concat') -> None:
        super().__init__()

        batch_first = True

        self.model_type = 'MyTransformerClassifier'
        ...
        if self.pool == 'concat':
            self.classifier = nn.Sequential(
                nn.Linear(d_model*max_len, nclass)
            )
        elif self.pool in ['max', 'mean']:
    ...
    def forward(self, src) -> Tensor:
        ...
        if self.pool == 'concat':
            output = output.reshape(batch_size, -1)  # batch, seq * d_model
        elif self.pool == 'max':
